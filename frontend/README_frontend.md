...
- 2026-08-01 17:58 +07 - Đối chiếu lại sau p2 hoàn chỉnh: frontend vẫn gọi `/api/chat/openai`; route này sau đó đã được cập nhật để dùng hybrid retrieval + BM25 + reranker + `ContextBuilder`.
...
Endpoint này sau p2 đã dùng hybrid retrieval, BM25 và reranker, nhưng gọi legacy `llm/generator.py`. Frontend hiện không gọi endpoint này vì đang dùng endpoint OpenRouter `/api/chat/openai`, route cũng đã dùng hybrid retrieval, BM25, reranker và `ContextBuilder`.
